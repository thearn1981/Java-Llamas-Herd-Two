...
public class SalesManager {
    ...
    static Customer selectCustomerForEditing(ImageIcon logo) {
        if (customers.size() > 15) {
            ...
            if (choice == 0) {
                // Search mode
                String customerName = Main.searchAndSelectCustomer(logo);
    ...
    static void customersMenu(ImageIcon logo) {
        while (true) {
            ...
            bPoints.addActionListener(_ -> { pane.setValue("POINTS"); close.run(); });
    ...
    while (true) {
        ...
        // ===== Layout rows =====
        // Row 1: New Invoice, Tax Rate (CENTER aligned)
        JPanel row1 = new JPanel(new FlowLayout(FlowLayout.CENTER, 12, 0));
        row1.add(bNew); row1.add(bTax);
        row1.setAlignmentX(Component.CENTER_ALIGNMENT);

        // Row 2: List Invoices, Back (CENTER aligned)
        JPanel row2 = new JPanel(new FlowLayout(FlowLayout.CENTER, 12, 0));
        ...
        JDialog dlg = pane.createDialog(null, "Invoices Menu");
        dlg.setModal(true);
        dlg.setResizable(false);

        // ===== Button actions =====
        Runnable close = dlg::dispose;
        bNew.addActionListener(_  -> { pane.setValue("NEW");  close.run(); });
        bList.addActionListener(_ -> { pane.setValue("LIST"); close.run(); });
        bTax.addActionListener(_  -> { pane.setValue("TAX");  close.run(); });
        bBack.addActionListener(_ -> { pane.setValue("BACK"); close.run(); });
    ...
    static void newInvoice(ImageIcon logo) {
        ...
        if (pointsDiscount > 0) {
            summary.append(String.format("%nPoints used: %d (Discount: $%.2f)", pointsUsed, pointsDiscount));
        }
