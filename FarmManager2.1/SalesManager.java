...
public class SalesManager {
    ...
    static class Customer {
        ...
        static Customer fromCSV(String line) {
            // Try tab-separated first, then comma-separated for backward compatibility
    ...
    static void loadCustomers() {
        customers.clear();
        var lines = Main.Store.readLines(Main.Store.CUST_FILE);
    ...
    static void loadInvoices() {
        invoices.clear();
        var lines = Main.Store.readLines(Main.Store.INV2_FILE);
        for (int i = 0; i < lines.size(); i++) {
            // Skip the first line (header row)
            if (i == 0) continue;
    ...
    while (true) {
        ...
        // ===== Buttons =====
        JButton bNew  = new JButton("Create New Invoice");
        JButton bList = new JButton("List Invoices");
        JButton bBack = new JButton("Back");

        // ===== Uniform button sizes =====
        JButton[] all = { bNew, bList, bBack };
        int maxW = 0, maxH = 0;
        for (JButton b : all) {
            Dimension d = b.getPreferredSize();
            maxW = Math.max(maxW, d.width);
