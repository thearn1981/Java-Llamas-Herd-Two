...
public class UI {
    ...
    public static String[] wrapText(String text, int maxWidth) {
        ...
        for (String line : explicitLines) {
            ...
            // For long lines, wrap based on sentence structure
            ...
            for (int i = 0; i < sentences.length; i++) {
                ...
                if (i < sentences.length - 1) {
                    ...
                    int sentenceStart = line.indexOf(sentence, currentLine.length() > 0 ? 
                        line.indexOf(currentLine.toString()) + currentLine.length() : 0);
                    if (sentenceStart >= 0 && sentenceStart + sentence.length() < line.length()) {
                        char lastChar = line.charAt(sentenceStart + sentence.length());
                        sentence += lastChar + " ";
                    }
                }
            ...
            // Add the last line for this explicit line
     ...
     * -------------------
     * Centralized UI methods for all manager menus.
     * Each method returns the user's choice as a string,
     * allowing the manager classes to focus on business logic.
     */

    /**
     * Animals Menu Interface
     */
    public static String showAnimalsMenu(ImageIcon logo) {
        ...
        return showManagerDialog(root, "Animals Menu", new JButton[]{bAdd, bEdit, bRem, bList, bBack}, 
                                new String[]{"ADD", "EDIT", "REM", "LIST", "BACK"});
    }

    /**
     * Inventory Menu Interface
     */
    public static String showInventoryMenu(ImageIcon logo) {
        // Calculate responsive dimensions
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        int menuWidth = Math.min(500, (int)(screenSize.width * 0.4));
        int menuHeight = Math.min(400, (int)(screenSize.height * 0.5));
        ...
        return showManagerDialog(root, "Inventory Menu", new JButton[]{bAdd, bEdit, bRemove, bList, bBack}, 
                                new String[]{"ADD", "EDIT", "REM", "LIST", "BACK"});
    }
     ...
     */
    public static String showServicesMenu(ImageIcon logo) {
        // Calculate responsive dimensions
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        int menuWidth = Math.min(500, (int)(screenSize.width * 0.4));
        int menuHeight = Math.min(400, (int)(screenSize.height * 0.5));
        ...
        return showManagerDialog(root, "Services Menu", new JButton[]{bAdd, bEdit, bRem, bList, bPaid, bBack}, 
                                new String[]{"ADD", "EDIT", "REM", "LIST", "PAID", "BACK"});
    }

    /**
     * Customers Menu Interface
     */
    public static String showCustomersMenu(ImageIcon logo) {
        // Calculate responsive dimensions
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        int menuWidth = Math.min(500, (int)(screenSize.width * 0.4));
        int menuHeight = Math.min(400, (int)(screenSize.height * 0.5));
        ...
        return showManagerDialog(root, "Customers Menu", new JButton[]{bAdd, bEdit, bRem, bList, bPoints, bBack}, 
                                new String[]{"ADD", "EDIT", "REM", "LIST", "POINTS", "BACK"});
    }

    /**
     * Invoices Menu Interface
     */
    public static String showInvoicesMenu(ImageIcon logo) {
        // Calculate responsive dimensions
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        int menuWidth = Math.min(500, (int)(screenSize.width * 0.4));
        int menuHeight = Math.min(400, (int)(screenSize.height * 0.5));
        ...
        JButton bList = new JButton("List Invoices");
        JButton bTax  = new JButton("Tax Rate");
        JButton bBack = new JButton("Back");

        // Uniform button sizes
        JButton[] all = { bNew, bList, bTax, bBack };
        standardizeButtonSizes(all);

        // Row 1: New, List
        JPanel row1 = new JPanel(new FlowLayout(FlowLayout.CENTER, 12, 0));
        row1.add(bNew); row1.add(bList);
        row1.setAlignmentX(Component.CENTER_ALIGNMENT);

        // Row 2: Tax, Back
        JPanel row2 = new JPanel(new FlowLayout(FlowLayout.CENTER, 12, 0));
        row2.add(bTax); row2.add(bBack);
        row2.setAlignmentX(Component.CENTER_ALIGNMENT);
        ...
        return showManagerDialog(root, "Invoices Menu", new JButton[]{bNew, bList, bTax, bBack}, 
                                new String[]{"NEW", "LIST", "TAX", "BACK"});
    }

    /**
     * Generic Manager Dialog Handler
     * Helper method to create and display manager dialogs with consistent behavior.
     */
    private static String showManagerDialog(JPanel content, String title, JButton[] buttons, String[] values) {
        // Create JOptionPane with custom content
        ...
        return (val == null) ? "BACK" : String.valueOf(val);
    }
}